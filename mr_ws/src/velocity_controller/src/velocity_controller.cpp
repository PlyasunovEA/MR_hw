...
double past_current_velocity = 0;
...
void on_odo(const nav_msgs::Odometry& odom)
{
  ...
  //current_velocity = 0.1*current_velocity+0.9*past_current_velocity;
  past_current_velocity = current_velocity;
...
void on_timer(const ros::TimerEvent& event) {
  ...
  throttle_cmd.data = (desired_velocity - current_velocity)*50+10*((desired_velocity - current_velocity)/0.1);
